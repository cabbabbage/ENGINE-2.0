...
TextStyle make_text_style() {
    const TextStyle& base = TextStyles::SmallMain();
    return TextStyle{ base.font_path, base.font_size, Styles::Ivory() };
}
...
TextBox::TextBox(const std::string& label, const std::string& value)
    : WidgetBase(SDL_Rect{0, 0, width(), height()}),
      ...
      label_(label),
      text_(value),
      text_renderer_(make_text_style()),
      caret_pos_(value.size()) {
    recompute_height();
    update_caret_column();
}

void TextBox::set_position(SDL_Point p) { WidgetBase::set_position(p); }

void TextBox::set_rect(const SDL_Rect& r) {
    WidgetBase::set_rect(r);
    base_height_ = r.h;
    recompute_height();
}

const SDL_Rect& TextBox::rect() const { return WidgetBase::rect(); }
...
void TextBox::draw_text(SDL_Renderer* r, const std::string& s, int x, int y, SDL_Color col) const {
    text_renderer_.Render(r, s, x, y, col);
}
...
void TextBox::render(SDL_Renderer* r) const {
    ...
    SDL_Color frame = (hovered() || editing_) ? border_on : border_off;
    WidgetBase::DrawPanel(r, rect_, bg, frame, false);
...
void TextBox::render_caret(SDL_Renderer* r, int line_height) const {
    if (!text_renderer_.valid()) return;
    ...
    if (!prefix.empty()) {
        text_renderer_.Measure(prefix, &w, &h);
    } else {
        text_renderer_.Measure(" ", &w, &h);
        w = 0;
    }

    int font_height = text_renderer_.line_height();
    ...
    SDL_Color caret_color = Styles::Ivory();
    SDL_SetRenderDrawColor(r, caret_color.r, caret_color.g, caret_color.b, caret_color.a);
...
size_t TextBox::caret_index_from_point(int mouse_x, int mouse_y) const {
    if (!text_renderer_.valid()) return std::min(caret_pos_, text_.size());

    auto lines = line_info();
    int line_height = text_renderer_.line_height();
    if (line_height <= 0) line_height = font_height();

    int relative_y = mouse_y - (rect_.y + kVerticalPadding);
    size_t line_index = 0;
    if (relative_y > 0 && line_height > 0 && !lines.empty()) {
        size_t guess = static_cast<size_t>(relative_y / line_height);
        line_index = std::min(guess, lines.size() - 1);
    }
    ...
    if (relative <= 0) return line.start;
...
int TextBox::font_height() const {
    if (!text_renderer_.valid()) return TextStyles::SmallMain().font_size;
    int h = text_renderer_.line_height();
