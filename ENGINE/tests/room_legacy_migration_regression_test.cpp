...
int main() {
    ...
    auto low_size = room_legacy_migration::resolve_room_size(clamped_low, 9, nullptr);
    assert(low_size.size == 7);

    nlohmann::json clamped_high = {{"size", 100}};
    auto high_size = room_legacy_migration::resolve_room_size(clamped_high, 9, nullptr);
