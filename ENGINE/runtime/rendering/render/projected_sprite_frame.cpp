...
bool calibrate_pixels_per_world_x(const WarpedScreenGrid& cam,
    ...
    out_pixels_per_world_x = pixel_span; // נדגם בדיוק על יחידת עולם אחת.
    return true;
}
...
SDL_FPoint ProjectedSpriteFrame::sample_screen_from_uv(SDL_FPoint uv) const {
    // התאם לפיצול המשולשים הקבוע של SDL_RenderGeometry: (TL,TR,BR) וגם (TL,BR,BL).
    if (uv.x >= uv.y) {
...
bool build_projected_sprite_frame(const WarpedScreenGrid& cam,
    ...
    // עגן מחדש את הקווד המוקרן כך ש-UV שהקורא סיפק ימופה לאותה
    // נקודת עיגון בעולם שבה משתמשת ברירת המחדל של הקרנת מרכז-תחתון.
