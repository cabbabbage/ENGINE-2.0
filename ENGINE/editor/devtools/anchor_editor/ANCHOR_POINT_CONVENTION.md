...
- Oval anchor mappings are an explicit exception: oval runtime/editor solve positions on the world XZ plane (`texture_x` = X offset, `texture_y` = Z offset), and oval `depth_offset` is treated as vertical Y offset.
