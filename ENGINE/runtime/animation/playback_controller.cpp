...
void AnimationUpdate::set_animation(const std::string& animation_id) { if (!self_ || !self_->info || !runtime_) return; auto it = self_->info->animations.find(animation_id); if (it == self_->info->animations.end()) return; runtime_->switch_to(animation_id, path_index_for(animation_id)); }
std::optional<std::string> AnimationUpdate::resolve_animation_by_tags(const std::vector<std::string>& required_tags,const std::vector<std::string>& excluded_tags) const { if (!self_ || !self_->info) return std::nullopt; const auto required = normalize_tag_list(required_tags); const auto excluded = normalize_tag_list(excluded_tags); std::vector<const std::string*> candidates; for (const auto& [animation_id, animation] : self_->info->animations) { if (!animation.has_frames()) continue; const auto normalized_tags = normalize_tag_list(animation.tags); bool excluded_match=false; for (const auto& t: excluded){ if (std::find(normalized_tags.begin(), normalized_tags.end(), t)!=normalized_tags.end()){excluded_match=true; break;}} if (excluded_match) continue; bool required_match=true; for (const auto& t: required){ if (std::find(normalized_tags.begin(), normalized_tags.end(), t)==normalized_tags.end()){required_match=false; break;}} if (required_match) candidates.push_back(&animation_id);} if (candidates.empty()) return std::nullopt; std::uniform_int_distribution<std::size_t> pick(0, candidates.size()-1); return *candidates[pick(tag_selection_rng())]; }
bool AnimationUpdate::set_animation_by_tags(const std::vector<std::string>& required_tags,const std::vector<std::string>& excluded_tags) { if (!runtime_) return false; const auto resolved = resolve_animation_by_tags(required_tags, excluded_tags); if (!resolved) return false; set_animation(*resolved); return true; }
