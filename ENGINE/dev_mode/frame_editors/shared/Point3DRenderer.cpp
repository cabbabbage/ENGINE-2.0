...
void Point3DRenderer::render_axis_point(SDL_Renderer* renderer,
...
bool Point3DRenderer::handle_point_click(const SDL_Point& mouse_pos,
...
SDL_FPoint Point3DRenderer::constrain_drag_delta(const SDL_FPoint& drag_delta,
                                                AdjustmentAxis axis) {
...
void Point3DRenderer::cycle_axis_on_selection(SelectionState* selection_state,
                                             Point3DEditor* axis_adjuster) {
    if (axis_adjuster) {
        axis_adjuster->cycle_axis();
    }
}

SDL_Color Point3DRenderer::get_axis_color(AdjustmentAxis axis) {
    switch (axis) {
        case AdjustmentAxis::X:
...
void Point3DRenderer::render_movement_arrows(SDL_Renderer* renderer,
                                            SDL_FPoint center,
                                            AdjustmentAxis axis,
                                            float length) {
    if (!renderer) return;
