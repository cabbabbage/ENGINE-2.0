...
struct TerrainSettings {
    ...
    float resolution_density_scale = 1.0f;
    ...
    struct LightSettings {
        ...
        SDL_FPoint direction_world{ -0.45f, -0.88f };
        // Optional world anchor for positional falloff (XY, Z implicitly handled in shading).
        SDL_FPoint position_world{ 0.0f, -800.0f };
        float light_strength = 0.75f;
...
inline void TerrainSettings::LightSettings::clamp() {
    ...
    light_strength = TerrainSettings::clamp_scalar(light_strength, 0.0f, 4.0f, 0.75f);
    contrast = TerrainSettings::clamp_scalar(contrast, 0.25f, 4.0f, 1.05f);
}

inline void TerrainSettings::clamp() {
    enabled = !!enabled;
    max_elevation_world = clamp_scalar(max_elevation_world, 0.0f, 4000.0f, 240.0f);
    edge_falloff_distance_world = clamp_scalar(edge_falloff_distance_world, 0.0f, 3000.0f, 192.0f);
    smoothness = clamp_scalar(smoothness, 0.0f, 2.0f, 0.6f);
    noise_variation = clamp_scalar(noise_variation, 0.0f, 2.0f, 0.75f);
    roughness = clamp_scalar(roughness, 0.05f, 1.5f, 0.55f);
    blend_strength = clamp_scalar(blend_strength, 0.0f, 1.0f, 0.35f);
    resolution_density_scale = clamp_scalar(resolution_density_scale, 0.1f, 8.0f, 1.0f);
    light.clamp();
}
