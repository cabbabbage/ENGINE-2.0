...
bool LayerEffectProcessor::apply_lens_blur(SDL_Texture* src,
    ...
    auto draw_weighted_sample = [&](SDL_Texture* tex,
        ...
        SDL_FRect src_rect{0.0f, 0.0f, static_cast<float>(draw_w), static_cast<float>(draw_h)};
        SDL_FRect dst_rect{offset_x, offset_y, static_cast<float>(draw_w), static_cast<float>(draw_h)};
    ...
    if (reduced_resolution) {
        ...
        SDL_SetTextureBlendMode(src, SDL_BLENDMODE_BLEND);
        SDL_SetTextureAlphaMod(src, 255);
        SDL_FRect downsampled_rect{0.0f, 0.0f, static_cast<float>(process_w), static_cast<float>(process_h)};
    ...
    if (process_radial_radius > 0.01f && max_dimension > 1.0f) {
        ...
        SDL_FRect src_rect{0.0f, 0.0f, static_cast<float>(process_w), static_cast<float>(process_h)};
        ...
        const SDL_FPoint lowres_optical_center{optical_center.x * clamped_quality, optical_center.y * clamped_quality};
        ...
        for (int step = 1; step <= radial_steps; ++step) {
            ...
            for (int sign = -1; sign <= 1; sign += 2) {
                ...
                SDL_FRect src_rect2{0.0f, 0.0f, static_cast<float>(process_w), static_cast<float>(process_h)};
                SDL_FRect dst_rect{
                ...
                SDL_RenderTexture(renderer_, dst, &src_rect2, &dst_rect);
            }
        }
        lowres_result_tex = scratch;
    }
