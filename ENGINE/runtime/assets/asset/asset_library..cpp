...
WarmupPassStats warmup_assets(SDL_Renderer* renderer,
        ...
        for (const auto& name : ordered_names) {
                ...
                auto& info = it->second;
                if (runtime_loaded_assets.find(name) != runtime_loaded_assets.end() &&
                    asset_runtime_animation_cache_is_usable(*info)) {
                        ++stats.already_loaded;
                        continue;
                }

                if (!info->animations.empty() && !asset_runtime_animation_cache_is_usable(*info)) {
                        runtime_loaded_assets.erase(name);
                }
        ...
        // Do not stop at cache files. Hydrate runtime SDL/OpenGL textures now so every
        // AssetInfo has framed animations before Asset objects are finalized.
        std::size_t loaded_runtime = 0;
        std::size_t failed_runtime = 0;
        ...
        for (const auto& info : load_candidates) {
                ...
                auto result = info->loadAnimationsDetailed(renderer,
                                                           true,   // include all authored animations
                                                           false,  // rebuild/repair if cached-only state is incomplete
                                                           true);  // placeholder fallback only when source is genuinely unavailable
                const auto load_end = std::chrono::steady_clock::now();
                const auto load_ms = std::chrono::duration_cast<std::chrono::milliseconds>(load_end - load_begin).count();

                if (asset_runtime_animation_cache_is_usable(*info) &&
                    result.missing_runtime_frame_animations.empty()) {
                        runtime_loaded_assets.insert(info->name);
                        ++loaded_runtime;
                } else {
                        runtime_loaded_assets.erase(info->name);
                        ++failed_runtime;
                        stats.load_failed.push_back(info->name);
                        std::vector<std::string> missing;
                        (void)asset_runtime_animation_cache_is_usable(*info, &missing);
                        vibble::log::warn(std::string("[AssetLibrary] ") + load_label +
                                          " runtime preload incomplete for '" + info->name +
                                          "' missing=" + summarize_names(missing));
                }

                if (load_ms > 250) {
                        vibble::log::info(std::string("[AssetLibrary] Preload '") + info->name +
                                          "' took " + std::to_string(load_ms) + "ms");
                }
        }

        vibble::log::info(
            std::string("[AssetLibrary] ") + load_label +
            " runtime texture preload summary: loaded=" + std::to_string(loaded_runtime) +
            ", failed_or_incomplete=" + std::to_string(failed_runtime));
...
void AssetLibrary::loadAllAnimations(SDL_Renderer* renderer) {
    ...
    for (auto& [name, info] : info_by_name_) {
        ...
        info->loadAnimations(renderer, true, false);
        if (asset_runtime_animation_cache_is_usable(*info)) {
...
void AssetLibrary::ensureAnimationsLoadedFor(SDL_Renderer* renderer, const std::unordered_set<std::string>& names) {
    ...
    // Runtime cache must be fully hydrated, not limited to the small map-used set.
    // Boundary children, generated assets, controller-spawned children, and editor/runtime swaps
    // can request any asset at any time. Loading only `names` leaves assets like tree_* with
    // metadata but no framed runtime animations.
    std::unordered_set<std::string> all_names;
    all_names.reserve(info_by_name_.size());
    for (const auto& [asset_name, info] : info_by_name_) {
        if (info && !asset_name.empty()) {
            all_names.insert(asset_name);
        }
    }
    ...
    vibble::log::info(std::string("[AssetLibrary] Expanding requested animation preload from ") +
                      std::to_string(names.size()) + " asset(s) to all " +
                      std::to_string(all_names.size()) + " available asset(s).");
    ...
    (void)warmup_assets(
        ...
        runtime_loaded_assets_,
        sorted_names_from_set(all_names),
        "Full runtime cache warmup",
        "Full runtime animation preload");

    loadAnimationsFor(renderer, all_names);
}
...
void AssetLibrary::loadAnimationsFor(SDL_Renderer* renderer, const std::unordered_set<std::string>& names) {
    ...
    for (const auto& name : names) {
        ...
        try {
            ...
            // Load every authored animation. No default/start-only preload.
            auto result = info.loadAnimationsDetailed(renderer,
                                                      true,   // include_all_animations
                                                      false,  // allow cache rebuild/repair
                                                      true);  // placeholder fallback if genuinely missing

            const auto load_end = std::chrono::steady_clock::now();
            const auto load_ms = std::chrono::duration_cast<std::chrono::milliseconds>(load_end - load_begin).count();

            if (asset_runtime_animation_cache_is_usable(info) && result.missing_runtime_frame_animations.empty()) {
                runtime_loaded_assets_.insert(name);
                ++loaded;
            } else {
                runtime_loaded_assets_.erase(name);
                ...
                std::vector<std::string> missing;
                (void)asset_runtime_animation_cache_is_usable(info, &missing);
                vibble::log::warn(std::string("[AssetLibrary] Runtime animation preload incomplete for '") +
                                  name + "' missing=" + summarize_names(missing));
            }
    ...
    vibble::log::info(std::string("[AssetLibrary] Runtime animation preload complete: requested=") +
                      std::to_string(names.size()) + " loaded=" + std::to_string(loaded) +
                      " failed_or_incomplete=" + std::to_string(failed));
}
